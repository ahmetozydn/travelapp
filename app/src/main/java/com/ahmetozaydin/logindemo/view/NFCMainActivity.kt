...
import com.ahmetozaydin.logindemo.databinding.ActivityNfcmainAcitivityBinding


import kotlinx.android.synthetic.main.activity_main.*
import kotlinx.android.synthetic.main.activity_nfcmain_acitivity.*
...
class NFCMainActivity : AppCompatActivity() {
    ...
    private lateinit var binding : ActivityNfcmainAcitivityBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityNfcmainAcitivityBinding.inflate(layoutInflater)
        ...
        try {
            ...
            if (nfcAdapter == null) {
                val builder = AlertDialog.Builder(this@NFCMainActivity, R.style.MyAlertDialogStyle)
                builder.setMessage("This device doesn't support NFC.")
                builder.setPositiveButton("Cancel", null)
                val myDialog = builder.create()
                myDialog.setCanceledOnTouchOutside(false)
                myDialog.show()
                txtviewshopid.text = "THIS DEVICE DOESN'T SUPPORT NFC. PLEASE TRY WITH ANOTHER DEVICE!"
                txtviewmachineid.visibility = View.INVISIBLE
            } else if (!nfcAdapter!!.isEnabled) {
                val builder = AlertDialog.Builder(this@NFCMainActivity, R.style.MyAlertDialogStyle)
                builder.setTitle("NFC Disabled")
                builder.setMessage("Plesae Enable NFC")
                txtviewshopid.text = "NFC IS NOT ENABLED. PLEASE ENABLE NFC IN SETTINGS->NFC"
                txtviewmachineid.visibility = View.INVISIBLE
                    ...
                    if (!txtuserid.text.toString().equals("")) {
                        if (NfcAdapter.ACTION_TECH_DISCOVERED == intent.action
                            ...
                            if (ndef.isWritable) {
                                ...
                                binding.txtviewuserid.text = "USER ID: "+txtuserid.text.toString();
                                Toast.makeText(
                                    applicationContext,
                                    "Successfully Wroted!",
                                    Toast.LENGTH_SHORT
